...
import com.chronosx.bikeshop.mapper.OrderDetailMapper;
import com.chronosx.bikeshop.mapper.OrderMapper;
import com.chronosx.bikeshop.mapper.PaymentMethodMapper;
import com.chronosx.bikeshop.repository.OrderRepository;
import com.chronosx.bikeshop.repository.PaymentMethodRepository;
import com.chronosx.bikeshop.repository.ProductDetailRepository;
import com.chronosx.bikeshop.service.OrderService;
import com.chronosx.bikeshop.utility.MailContent;
...
public class OrderServiceImpl implements OrderService {
    ...
    public List<OrderResponse> findAllOrdersInCurrentMonth() {
        ...
        return orderRepository.findOrdersInMonth(startOfMonth, endOfMonth).stream()
    ...
    public OrderResponse addNewOrder(CreateNewOrderRequest request) throws IOException {
        ...
        rawOrders.setPaymentMethod(paymentMethodRepository
        ...
        List<OrderDetail> orderDetailList = request.getCreateNewOrderDetailDtoList().stream()
                .map(dto -> {
                    ProductDetail productDetail = productDetailRepository
                            .findById(dto.getProductDetailId())
        ...
        if (request.getPaymentMethodId() == 1) {
            ...
            notificationServiceImpl.sendOrderMail(
                    order.getEmail(), MailContent.pendingSubject, MailContent.htmlPendingContent, order.getId());
        ...
        if (request.getPaymentMethodId() == 2) {
            ...
            for (OrderDetail orderDetail : orderDetailList) {
                totalPrice =
                        totalPrice + orderDetail.getProductDetail().getProduct().getPrice() * orderDetail.getAmount();
            }
    ...
    public OrderResponse setPendingAfterPaidById(Long id, String token) {
        if (paypalService.checkPaymentStatus(token)) {
            Orders order =
                    orderRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.ORDER_ID_NOT_FOUND));
    ...
    public OrderResponse confirmOrderById(Long id) {
        ...
        notificationServiceImpl.createNewNotificationWhenCreateNewOrder(order);
        notificationServiceImpl.sendOrderMail(
                order.getEmail(), MailContent.confirmedSubject, MailContent.htmlPendingContent, order.getId());
        notificationServiceImpl.sendNotificationByWebSocket(order);
        return toOrderResponse(orderRepository.save(order));
    }
    ...
    public OrderResponse denyOrderById(Long id) {
        ...
        notificationServiceImpl.createNewNotificationWhenCreateNewOrder(order);
        notificationServiceImpl.sendOrderMail(
                order.getEmail(), MailContent.deniedSubject, MailContent.htmlPendingContent, order.getId());
        notificationServiceImpl.sendNotificationByWebSocket(order);
        return toOrderResponse(orderRepository.save(order));
    }
    ...
    private OrderResponse toOrderResponse(Orders orders) {
        ...
        orderResponse.setOrderDetailResponseList(orders.getOrderDetails().stream()
    ...
    private OrderDetailResponse toOrderDetailResponse(OrderDetail orderDetail) {
        ...
        orderDetailResponse.setProductDetailResponse(
                productDetailServiceImpl.toProductDetailResponse(orderDetail.getProductDetail()));
